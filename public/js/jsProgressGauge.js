(function (a) {
	...
	a.fn.JsProgressGauge = function (b) {
		...
		var c = function (d) {
			...
			d = d > 100 ? 100 : d < 0 ? 0 : d;
			b.value = d;
			b.labelFormatter != null &&
				typeof b.labelFormatter == "function" &&
				(c = b.labelFormatter(d, a) || ""),
				b.animated
					? e.animate(
							{
							...
							},
							{
								step: function (d) {
									b.showLabel &&
										(c != null
											? a.html(c)
											: a.html(Math.round(d) + b.labelUnit));
								},
								duration: b.duration,
								complete:
									typeof b.onFinish == "function" && d >= 100
										? k.bind(this, b)
										: null,
							}
					  )
					: (e.css(b.type == "column" ? "height" : "width", d + "%"),
					  c != null ? a.html(c) : a.html(Math.round(d) + "%"));
		};
		var d = this.each(function (B, d) {
			a(d).data("progress-init") === undefined &&
				a(d).data("progress-init", b.value);
			let z = a(d).data("progress-init");
			z === b.value &&
				a(d).html(
					'<div class="progressGauge-boxbar">' +
						'<div class="progressGauge-progressbar">' +
						'<div class="progressGauge-progress"></div>' +
						"</div>" +
						'<div class="progressGauge-displaybox">' +
						'<span class="progressGauge-displayTitle"></span>' +
						'<span class="progressGauge-displayLabel"></span>' +
						"</div>" +
						"</div>"
				);
			...
			b.showTitle &&
				b.title != null &&
				b.title.length > 0 &&
				(m.html(b.title), (u = "space-between"));
			var n = "center";
			var w = b.titleFontSize != null ? b.titleFontSize : b.labelFontSize;
			var x =
				+b.labelFontSize > +b.titleFontSize
					? +b.labelFontSize
					: +b.titleFontSize;
			var A = "width";
			if (b.type == "column") {
				...
				var q = 0;
				b.labelAlignment == "left"
					? (q = -(p + 10))
					: b.labelAlignment == "right" && (q = p + 10),
					h.css({
				...
				var y = k < 0 ? g : i;
				var r = 0;
				(b.labelPosition == "top" || b.labelPosition == "bottom") &&
					(b.showLabel || b.showTitle) &&
					((y += x + 6),
					b.labelPosition == "top"
						? ((n = "flex-end"), (r += x + 10))
						: (n = "flex-start")),
...
setInterval(() => {
	$(document).ready(function () {
		let gauge1Options = {
			animated: true,
			backgroundColor: "rgba(22,39,80, 0.8)",
			barHeight: "8px",
			barRadius: 4,
			boldLabels: true,
			duration: 1000,
			fillBackgroundColor:
				"linear-gradient(90deg,  #ff3311 50%, #88ff22 90%, #11ff55 100%)",
			fillRadius: 4,
			fillSize: "8px",
			...
			titleFontSize: null,
			type: "line",
			value: dashboardData.ACfrequency || 0,
			// value: 95,
			width: "100%",
		};
		$("#frequency-progress-bar").JsProgressGauge(gauge1Options);
	});
	...
	$(document).ready(function () {
		let gauge1Options = {
			...
			titleFontSize: null,
			type: "line",
			value: dashboardData.ACfirstTotalActivePower || 0,
			// value: 50,
			width: "100%",
		};
		$("#power-progress-bar").JsProgressGauge(gauge1Options);
	});
	...
	$(document).ready(function () {
		let gauge1Options = {
			...
			titleFontSize: null,
			type: "line",
			value: dashboardData.runHour || 0,
			// value: 90,
			width: "100%",
		};
		$("#run-hour-progress-bar").JsProgressGauge(gauge1Options);
	});
	...
	$(document).ready(function () {
		let gauge1Options = {
			...
			titleFontSize: null,
			type: "line",
			value: dashboardData.energy || 0,
			// value: 80,
			width: "100%",
		};
		$("#energy-progress-bar").JsProgressGauge(gauge1Options);
	});
}, 2500);
